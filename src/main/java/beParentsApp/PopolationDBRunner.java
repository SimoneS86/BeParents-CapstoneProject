	...
	public void run(String... args) throws Exception {
		...
		for (int i = 0; i < 5; i++) {
			try {
				...
				ProfessionalUserRegistrationPayload user = new ProfessionalUserRegistrationPayload(name, surname, email,
						password, profession, registrationNumber, description);
		...
		for (Post post : createdPosts) {
			try {
				...
				LocalDateTime publicationDate1 = faker.date().past(30, java.util.concurrent.TimeUnit.DAYS).toInstant()
				...
				LocalDateTime publicationDate2 = faker.date().past(30, java.util.concurrent.TimeUnit.DAYS).toInstant()
						.atZone(ZoneId.systemDefault()).toLocalDateTime();
